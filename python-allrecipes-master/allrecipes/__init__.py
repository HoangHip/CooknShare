...
class AllRecipes(object):
	...
	def get(url):
		...
		name1 = soup.find("h1", {"class": "recipe-summary__h1"})
		if name1 != None:
			name = name1.get_text().replace("®", "")
		else:
			name = "None"

		direction_data = soup.find("div", {"class": "directions--section__steps"})
		
		prep_time1 = direction_data.find("time", {"itemprop": "prepTime"})
		if prep_time1 != None:
			prep_time = prep_time1.get_text()
		else:
			prep_time = "None"

		cook_time1 = direction_data.find("time", {"itemprop": "cookTime"})
		if cook_time1 != None:
			cook_time = cook_time1.get_text()
		else:
			cook_time = "None"
		
		total_time1 = direction_data.find("time", {"itemprop": "totalTime"})
		if total_time1 != None:
			total_time = total_time1.get_text()
		else:
			total_time = "None"
